...
import { getChapterData, getCustomSyllabus, getApiUsage, rtdb } from "../firebase";
// incrementApiUsage removed as client-side usage tracking by key index is no longer possible/relevant.
// Usage tracking should ideally move to server-side or be simplified.

// GROQ API CALL HELPER (SERVER PROXY)
export const callGroqApi = async (messages: any[], model: string = "llama-3.3-70b-versatile") => {
    // Send request to our backend proxy which handles keys and rotation
    ...
    if (!response.ok) {
        ...
        try {
            ...
            throw new Error(`Groq Proxy Error: ${errJson.error || response.status} - ${errJson.detail || errorText}`);
        } catch (e) {
    ...
    // The proxy returns the raw text content directly in response body for simplicity as per api/groq.ts implementation
    // "return new Response(text, ...)" where text is groqRes.text().
    // Wait, groqRes.text() returns the JSON string of the Groq response { choices: ... }.
    ...
    try {
        ...
        // Fallback if backend returned plain text or something else
...
export const executeWithRotation = async <T>(
    ...
    // Client-side rotation is REMOVED. Server handles it.
    // Quota check is simplified or removed as we don't know which key is used.
    // We just execute the operation.
...
// Simplified to just parallel execution without key rotation logic
const executeBulkParallel = async <T>(
    tasks: (() => Promise<T>)[],
    concurrency: number = 20, 
    usageType: 'PILOT' | 'STUDENT' = 'STUDENT'
): Promise<T[]> => {
    console.log(`🚀 Starting Bulk Engine (Groq Proxy): ${tasks.length} tasks (Parallelism: ${concurrency})`);
...
export const generateCustomNotes = async (userTopic: string, adminPrompt: string, modelName: string = "llama-3.3-70b-versatile"): Promise<string> => {
